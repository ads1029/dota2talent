      ...
      {page === 'archive' ? <>
        ...
        <section className="content">
          <div className="hero-heading">
            ...
            <div><div className={`eyebrow hero-attribute ${hero.primaryAttr}`}>{attributeLabel(hero.primaryAttr, language)} · {language==='zh'?`复杂度 ${hero.complexity}`:`COMPLEXITY ${hero.complexity}`}</div><h1>{language==='zh'?hero.zh:hero.en}</h1><p>{language==='zh'?hero.en:hero.zh} · <span>{hasTalentData ? version : language==='zh'?'该版本尚未登场':'NOT RELEASED IN THIS PATCH'}</span></p></div>
          ...
          <div className="source-card"><span><ShieldCheck size={18}/></span><div><b>{c.source}</b><p>{language==='zh'?`已载入 7.00–7.41e 全部 133 个版本；当前选择 ${version}，所有分支均保留中英文与变更记录。`:`All 133 patches from 7.00–7.41e are loaded. ${version} preserves bilingual branches and change records.`}</p></div><a href={version==='7.00'?'https://www.dota2.com/700/gameplay':`https://www.dota2.com/patches/${version}`} target="_blank">{c.official}<ExternalLink size={14}/></a></div>
        </section>
...
function VariantTalents({talents,language}:{talents:VariantTalent[],language:Language}) { if(!talents.length)return null; return <section className="variant-card"><div className="eyebrow">{language==='zh'?'命石专属天赋':'FACET-SPECIFIC TALENTS'}</div>{talents.map((talent,index)=><div className="variant-row" key={`${talent.facet}-${talent.level}-${index}`}><span>{talent.facet}</span><b>LV {talent.level}</b><p>{language==='zh'?talent.zh:talent.en}<small>{language==='zh'?talent.en:talent.zh}</small></p></div>)}</section> }

function Timeline({events,language}:{events:ArchiveEvent[],language:Language}) { return <div className="timeline">{events.length?events.map((event,i)=><article key={`${event.version}-${i}`}><div className="time-mark"><span/></div><div className="time-meta"><b>{event.version}</b><small>{event.facet ?? (language==='zh'?'英雄天赋':'HERO TALENT')}</small></div><div className="change-card"><span className="change-type">{event.type.replaceAll('_',' ')}</span><h3>{event.level?`Level ${event.level}`:(language==='zh'?'天赋变更':'Talent change')}</h3><p className="after">{language==='zh'?event.zh:event.en}</p><small>{language==='zh'?event.en:event.zh}</small></div></article>):<div className="empty"><Sparkles/>{language==='zh'?'该英雄没有单独的天赋变更事件':'No individual talent changes for this hero'}</div>}</div> }
