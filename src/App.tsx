      ...
      {page === 'archive' ? <>
        <section className="hero-rail">
          ...
          <div className="hero-list">{matchingHeroes.map(h => <button key={h.id} aria-label={language==='zh'?h.zh:h.en} className={h.id===heroId?'selected':''} onClick={()=>setHeroId(h.id)}>
            <span className="hero-mark" style={{'--hero':h.color} as React.CSSProperties}><img src={assetUrl(h.image)} alt=""/><i>{h.mark}</i></span><span><b>{language==='zh'?h.zh:h.en}</b></span><em className={`attr-dot ${h.primaryAttr}`}/>
          </button>)}</div>
...
function Timeline({events,language}:{events:ArchiveEvent[],language:Language}) { return <div className="timeline">{events.length?events.map((event,i)=><article key={`${event.version}-${i}`}><div className="time-mark"><span/></div><div className="time-meta"><b>{event.version}</b><small>{event.facet ?? (language==='zh'?'英雄天赋':'HERO TALENT')}</small></div><div className="change-card">{event.type!=='unknown'&&<span className="change-type">{event.type.replaceAll('_',' ')}</span>}<h3>{event.level?(language==='zh'?`${event.level} 级天赋`:`Level ${event.level}`):(language==='zh'?'天赋变更':'Talent change')}</h3><TimelineChange text={language==='zh'?event.zh:event.en} language={language}/></div></article>):<div className="empty"><Sparkles/>{language==='zh'?'该英雄没有单独的天赋变更事件':'No individual talent changes for this hero'}</div>}</div> }
...
function GenericIndex({language,version,setVersion,category,setCategory,sort,setSort,talents}:{language:Language,version:string,setVersion:(v:string)=>void,category:TalentCategory|'all',setCategory:(v:TalentCategory|'all')=>void,sort:'asc'|'desc',setSort:(v:'asc'|'desc')=>void,talents:Talent[]}) {
 ...
 <div className="talent-table"><div className="table-head"><span>#</span><span>{language==='zh'?'英雄':'HERO'}</span><span>{language==='zh'?'类别':'CATEGORY'}</span><span>{language==='zh'?'天赋':'TALENT'}</span><span>{language==='zh'?'等级':'LEVEL'}</span><span>{language==='zh'?'数值':'VALUE'}</span></div>{talents.map((t,i)=>{const h=heroes.find(x=>x.id===t.hero)!;return <div className="table-row" key={`${t.hero}-${t.level}-${t.side}`}><span className="rank">{String(i+1).padStart(2,'0')}</span><span className="table-hero"><i style={{'--hero':h.color} as React.CSSProperties}>{h.mark}</i><b>{language==='zh'?h.zh:h.en}</b></span><span><em>{categoryLabels[t.category!][language]}</em></span><span className="bilingual"><b>{language==='zh'?t.zh:t.en}</b></span><span>LV {t.level}</span><span className="value">{t.value}{t.unit==='percent'?'%':''}</span></div>})}{!talents.length&&<div className="empty"><Sparkles/>{c.empty}</div>}</div></section>
}
